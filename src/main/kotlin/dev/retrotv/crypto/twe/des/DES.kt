...
abstract class DES : TwoWayEncryption, KeyGenerator {
    ...
    companion object {
        ...
        protected const val BAD_PADDING_EXCEPTION_MESSAGE = ("BadPaddingException: "
                + "\n암호화 시 사용한 키와 일치하지 않습니다.")
        protected const val ILLEGAL_BLOCK_SIZE_EXCEPTION_MESSAGE = ("IllegalBlockSizeException: "
                + "\n암호화 되지 않은 데이터의 복호화를 시도중 이거나, 이미 다른 유형으로 인코딩 된 데이터의 암복호화를 시도하는 중인지 확인하십시오.")
        protected const val INVALID_ALGORITHM_PARAMETER_EXCEPTION_MESSAGE = ("InvalidAlgorithmParameterException: "
                + "\n%JAVA_HOME%\\jre\\lib\\security\\cacerts 파일이 존재하지 않거나 내부에 데이터가 존재하지 않는지 확인하십시오.")
        protected const val INVALID_KEY_EXCEPTION_MESSAGE = ("InvalidKeyException: "
        ...
        protected const val NO_SUCH_PADDING_EXCEPTION_MESSAGE = ("NoSuchPaddingException: "
                + "\n지원되지 않거나, 부정확한 포맷으로 패딩된 데이터를 암복호화 시도하고 있습니다.")
        protected const val NO_SUCH_ALGORITHM_EXCEPTION_MESSAGE = ("NoSuchAlgorithmException: "
