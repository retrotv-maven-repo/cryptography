...
class ARIA256CBCTest {
    private fun hexToBytes(hex: String): ByteArray =
        hex.chunked(2).map { it.toInt(16).toByte() }.toByteArray()

    @TestFactory
    fun test_ariaCbcKat(): Stream<DynamicTest> {
        val file = File("src/vector/ARIA/ARIA-256_(CBC)_KAT.txt")
        ...
        for (line in lines) {
            when {
                ...
                line.startsWith("CT =") -> {
                    ...
                    val testName = "ARIA-256-CBC KAT #$caseNum"
                    val testFn = {
                        val aria = ARIA()
                        val cbc = CBC(aria)
    ...
    fun test_ariaCbcMmt(): Stream<DynamicTest> {
        val file = File("src/vector/ARIA/ARIA-256_(CBC)_MMT.txt")
        val lines = file.readLines().map { it.trim() }.filter { it.isNotEmpty() }
        val tests = mutableListOf<DynamicTest>()
        ...
        for (line in lines) {
            when {
                ...
                line.startsWith("CT =") -> {
                    ct = line.substringAfter("=").trim()
                    val testName = "ARIA-256-CBC MMT #$caseNum"
                    val testFn = {
                        val aria = ARIA()
                        val cbc = CBC(aria)
                        val params = ParamWithIV(hexToBytes(key), hexToBytes(iv))
                        val result = cbc.encrypt(hexToBytes(pt), params).data
                        val resultHex = result.joinToString("") { "%02X".format(it) }
                        Assertions.assertEquals(ct.uppercase(), resultHex.substring(0, ct.length), "Failed at $testName")
                    }
                    tests.add(DynamicTest.dynamicTest(testName, testFn))
                    caseNum++
                }
    ...
    fun test_ariaCbcMct_allCounts(): Stream<DynamicTest> {
        val file = File("src/vector/ARIA/ARIA-256_(CBC)_MCT.txt")
        val lines = file.readLines().map { it.trim() }.filter { it.isNotEmpty() }
        ...
        return cases.map { mctCase ->
            DynamicTest.dynamicTest("ARIA-256-CBC MCT COUNT=${mctCase.count}") {
                val key = hexToBytes(mctCase.key)
                val iv = hexToBytes(mctCase.iv)
                val pts = Array(1000) { ByteArray(32) }
                val cts = Array(1000) { ByteArray(32) }
